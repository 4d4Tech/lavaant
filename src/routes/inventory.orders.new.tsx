...
import { callFunction, inventoryItems, usdCents } from "@/lib/mock-firebase";
...
function NewPurchaseOrder() {
  const [vendor, setVendor] = useState(inventoryItems[0].supplier);
  const [job, setJob] = useState(JOBS[0]);
  const [shipTo, setShipTo] = useState(SHIP_TO[0]);
  const [needBy, setNeedBy] = useState("");
  const [notes, setNotes] = useState("");
  const [lines, setLines] = useState<Line[]>([{ id: "itm_01", qty: 20 }]);
  ...
  const item = (id: string) => inventoryItems.find((i) => i.id === id)!;
  const subtotal = lines.reduce((s, l) => s + item(l.id).unitCost * l.qty, 0);
  ...
  const submit = async () => {
    ...
    const res = await callFunction("createPurchaseOrder", { vendor, job, shipTo, needBy, lines });
    setSubmitting(false);
    setSubmittedPo(res.id);
    toast.success(`${res.id} sent to ${vendor}`);
  };
