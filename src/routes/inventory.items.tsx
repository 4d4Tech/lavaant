...
import { inventoryItems, usdCents } from "@/lib/mock-firebase";
          ...
          <article key={i.id} className="surface p-4">
            ...
            {i.onHand < i.par ? (
              <p className="mt-3 text-xs font-bold text-destructive">
                Below par — reorder {i.par - i.onHand} {i.unit}
              </p>
            ) : null}
          </article>
        ))}
            ...
            <tbody>
              {rows.map((i) => (
                <tr key={i.id} className="hover:bg-muted/50">
                  ...
                  <Td
                    className={`text-right font-bold ${i.onHand < i.par ? "text-destructive" : ""}`}
                  >
                    {i.onHand} {i.unit}
                  </Td>
