...
import { callFunction, catalog, crmCustomers, usd, usdCents, vendorInbox } from "@/lib/mock-firebase";
...
function CrmOrders() {
  const [customer, setCustomer] = useState(crmCustomers[0].name);
  const [sku, setSku] = useState(catalog[0].id);
  const [qty, setQty] = useState(10);
  const [sending, setSending] = useState(false);

  const product = catalog.find((c) => c.id === sku)!;

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSending(true);
    const res = await callFunction("createOrderOnBehalf", { customer, sku, qty });
    setSending(false);
    toast.success(`${res.id} written for ${customer}`, {
      description: `${qty} × ${product.name} — ${usdCents(product.price * qty)}`,
    });
  };
  ...
  return (
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard label="Open orders" value="6" hint="Across your accounts" />
