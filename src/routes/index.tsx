...
import { DEMO_USERS, ROLE_META, type Role } from "@/lib/mock-firebase";
...
const ROLE_CARDS: { role: Role; icon: typeof Boxes; blurb: string }[] = [
  {
    role: "inventory",
    icon: Boxes,
...
function SignIn() {
  ...
  const [selected, setSelected] = useState<Role>("inventory");
    ...
    <div className="grid min-h-screen lg:grid-cols-[1.05fr_1fr]">
      ...
      <section className="gradient-brand relative flex flex-col justify-between px-6 py-10 text-sidebar-foreground sm:px-10 lg:px-14 lg:py-14">
        ...
        <p className="mt-10 text-xs text-sidebar-foreground/50">
          Scaffold build — Firebase Auth, Firestore, Storage & Functions are mocked.
        </p>
      </section>
      ...
      <section className="flex items-center justify-center px-4 py-10 sm:px-8 lg:px-12">
        <form onSubmit={handleSubmit} className="w-full max-w-md">
          ...
          <p className="mt-3 text-center text-xs text-muted-foreground">
            Demo credentials are prefilled. Any password works.
          </p>
        </form>
      </section>
    </div>
