...
import { ROLE_META, type Role } from "@/lib/mock-firebase";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/button";

type NavItem = { to: string; label: string; short: string; icon: typeof Boxes };
...
function PortalSwitcher({ current }: { current: Role }) {
  const { signIn } = useAuth();
  const navigate = useNavigate();
  const roles: Role[] = ["inventory", "vendor", "crm"];
  return (
    <div>
