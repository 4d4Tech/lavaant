/**
 * Mocked Firebase layer.
 *
 * Everything here mirrors the shape of the future Firebase Product Suite so the
 * swap is mechanical:
 *   - collections  -> Firestore documents (`firestore.collection(name).get()`)
 *   - assets       -> Firebase Storage download URLs (spec sheets / MSDS)
 *   - callables    -> Firebase Functions (headless, called from the client)
 *   - auth         -> Firebase Authentication (see src/lib/auth.tsx)
 */

export type Role = "inventory" | "vendor" | "crm";
...
export const ROLE_META: Record<Role, { name: string; tagline: string; home: string }> = {
  inventory: {
...
export type OrderStatus = "Draft" | "Submitted" | "Acknowledged" | "Shipped" | "Received";
...
export const vendorInbox: VendorOrder[] = [
  {
    id: "vo_01",
    po: "PO-10431",
    customer: "Ridgeline Builders",
...
export type CatalogProduct = {
  ...
  price: number;
  unit: string;
  stock: "In stock" | "Low" | "Backordered";
  specSheet: string;
  msds: string;
};

/** Storage paths resolve to Firebase Storage download URLs in production. */
...
/** Stand-in for a callable Firebase Function. Resolves after a short delay. */
export function callFunction<T>(name: string, payload: T, delay = 700) {
  return new Promise<{ ok: true; name: string; payload: T; id: string }>((resolve) => {
    setTimeout(
      () =>
        resolve({
          ok: true,
          name,
          payload,
          id: `PO-${Math.floor(10430 + Math.random() * 60)}`,
        }),
      delay,
    );
  });
